...
class jcalendar2 extends Controller{
	function jcalendar2(){
		...
		if($this->session->userdata('user')){
			...
			$this->userdetails = $this->db->get()->row_array();
		}else
			redirect('/login/logg');
	}
	...
	function index(){
		...
		$data['events'] = $events;
		$data['user'] = $this->user;
		$this->getcal($this->userdetails['rssfeed']);
		$sidedata['rss'] = $this->userdetails['rssfeed'];
	...
	function add(){
		...
		$db_groups = $this->Administration->user_member_of($this->user['userid']);
		...
		if($this->form_validation->run()){
			...
			$this->JCalendar->add_event($this->input->post('personal_event') ? $this->user['userid'] : null, $event_name, $start_date, $end_date, $event_details, $venue,$groups);
...
*/
		...
		$template['rss'] = $this->userdetails['rssfeed'];
	...
	function event($eventid){
		...
		$template['sidebar'] = 
			...
			anchor('jcalendar2/delete/' . $event['eventid'], 'delete event', array('onClick'=>"return (confirm('Are you sure you want to delete this event?'))")).
			br(1).
...
//, array('1' => array('groupid' => 2), '2' => array('groupid' => 1))
			$events = $this->JCalendar->select_events_by_criteria($this->user['userid'], $event_name, $start_date, $end_date, null, $groups);
		}
		$data['groups'] = $db_groups;
		$data['events'] = $events;
		$data['user'] = $this->user;
